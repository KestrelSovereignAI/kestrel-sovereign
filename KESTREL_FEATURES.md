...
- `GET /assets/{path}` — unauthenticated 307 redirect into `/phoenix/assets/{path}` for Phoenix's root-absolute dynamic-import chunks (no data served; auth enforced at the proxy target)
