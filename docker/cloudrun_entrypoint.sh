...
case "$PERSISTENCE_MODE" in
    durable_sovereign)
        ...
        if [ -z "${KESTREL_DATABASE_URL:-}" ] || [ -z "${KESTREL_HOLD_EVIDENCE_DATABASE_URL:-}" ] || [ -z "${KESTREL_HOLD_PAIR_ID:-}" ] || [ -z "${KESTREL_DATA_KEY:-}" ] || [ -z "${KESTREL_EXPECTED_DID:-}" ] || [ -z "${KESTREL_IDENTITY_BUNDLE:-}" ]; then
