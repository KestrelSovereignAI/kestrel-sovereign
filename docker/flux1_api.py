...
FLUX.1-specific implementation of the SimpleTuner API.
Handles FLUX.1-dev model configuration, quantization, and inference.
Includes multi-LoRA support for uncensored content generation.
"""
...
class FLUX1API(BaseSimpleTunerAPI):
    """FLUX.1-dev specific SimpleTuner API implementation with uncensored support."""

    def __init__(self):
        super().__init__(
            app_title="SimpleTuner FLUX.1 Training API",
            service_name="simpletuner-flux"
        )

        # FLUX.1-specific paths
        ...
        # Register FLUX.1 specific routes
        self._register_flux1_routes()
    ...
    def get_gcs_cache_prefix(self) -> str:
        return "model-cache/flux-dev-int8-quanto"
    ...
    def create_model_specific_config(self, base_config: dict) -> dict:
        """Create FLUX.1-specific configuration modifications."""
        # FLUX.1 specific quantization settings
        # Text encoders are unloaded before training starts, so no VRAM benefit from quantizing them
        base_config.update({
            # Quantize transformer to int8 for memory savings
            "base_model_precision": "int8-quanto",  # Quantize transformer to int8
            "text_encoder_1_precision": "no_change",  # CLIP - keep native precision
            "text_encoder_2_precision": "no_change",  # T5 - keep native precision
    ...
    def _load_inference_pipeline_impl(self, save_quantized: bool = True):
        ...
        if self.is_quantized_model_cached():
            logger.info("🚀 Loading CACHED quantized FLUX.1-dev from disk...")
            ...
            logger.info("✅ FLUX.1-dev loaded from CACHE (skip quantization)")
            return self._inference_pipeline

        # No cache - need to quantize from scratch
        logger.info("Loading FLUX.1-dev pipeline for inference (int8-quanto)...")
        ...
        logger.info("✅ FLUX.1-dev pipeline loaded (int8-quanto, ~50GB VRAM)")
        return self._inference_pipeline
    ...
    async def get_inference_pipeline(self, save_quantized: bool = True):
        """Lazy-load FLUX.1-dev pipeline for inference with int8-quanto quantization."""
    ...
    def _check_flux_cached(self) -> bool:
        """Check if FLUX.1-dev model is already fully cached."""
        try:
    ...
    def _register_flux1_routes(self):
        """Register FLUX.1 specific routes including uncensored support."""
        ...
        async def generate_image(
            ...
            """Generate images using FLUX.1-dev with a trained LoRA."""
            import base64
        ...
        async def generate_image_async(
            ...
            """Start async image generation with FLUX.1-dev."""
            import uuid
        ...
        async def preload_model(background_tasks: BackgroundTasks, quantize: bool = True):
            """Pre-download and optionally pre-quantize FLUX.1-dev model."""
            ...
            if quantize and self.is_quantized_model_cached():
                ...
                logger.info("Quantized FLUX.1-dev already cached, skipping")
                return {
                    "status": "already_cached",
                    "message": "Quantized FLUX.1-dev model is already cached on disk",
                    "cache_path": self.quantized_cache_dir,
                }

            # Check if base model is cached (partial case)
            if self._check_flux_cached() and not quantize:
                self._preload_status = {"status": "complete", "progress": "Model already cached!", "error": None}
                logger.info("FLUX.1-dev already cached, skipping download")
                return {"status": "already_cached", "message": "FLUX.1-dev model is already cached"}

            def download_and_quantize():
                try:
                    # Step 1: Download base model if needed
                    if not self._check_flux_cached():
                        self._preload_status = {"status": "running", "progress": "Downloading FLUX.1-dev (~60GB)...", "error": None}
                        from huggingface_hub import snapshot_download
                        logger.info("Starting FLUX.1-dev download...")
                        snapshot_download(
                            self.get_model_name(),
                            local_dir_use_symlinks=True
                        )
                        logger.info("FLUX.1-dev download complete!")
    ...
    async def run_vertex_batch_generation(self, args):
        """Run generation in batch mode for Vertex AI Custom Jobs with uncensored support."""
        from google.cloud import storage as gcs
        from io import BytesIO
        import sys
        import requests
        ...
        logger.info("=" * 60)
        logger.info("Vertex AI Batch Generation Mode - FLUX.1")
        logger.info("=" * 60)
        ...
        try:
            ...
            # Load character LoRA
            logger.info(f"Loading character LoRA from {local_lora_path}")
            pipe.load_lora_weights("/tmp/lora", weight_name="pytorch_lora_weights.safetensors", adapter_name="character")

            # Load uncensored LoRA for NSFW content (enhanceaiteam/Flux-Uncensored-V2)
            # This removes FLUX.1-dev's built-in content filtering
            uncensored_mode = os.environ.get("FLUX_UNCENSORED", "true").lower() == "true"
            if uncensored_mode:
                logger.info("Loading uncensored LoRA (enhanceaiteam/Flux-Uncensored-V2)...")
                try:
                    ...
                    # Combine both LoRAs - character at full strength, uncensored at 0.8
                    pipe.set_adapters(["character", "uncensored"], adapter_weights=[1.0, 0.8])
                    logger.info("✅ Both character and uncensored LoRAs loaded")
                except Exception as e:
                    logger.warning(f"Failed to load uncensored LoRA: {e}")
                    logger.info("Continuing with character LoRA only")
                    pipe.set_adapters(["character"], adapter_weights=[1.0])
            else:
                logger.info("Uncensored mode disabled, using character LoRA only")
                pipe.set_adapters(["character"], adapter_weights=[1.0])
            ...
            metadata = {
                ...
                "model": "FLUX.1-dev",
                "uncensored": uncensored_mode,
            }
            metadata_blob = output_bucket.blob(f"{output_prefix}/metadata.json")
            metadata_blob.upload_from_string(json.dumps(metadata, indent=2))

            logger.info("=" * 60)
            logger.info(f"FLUX.1 Generation completed! {len(images)} images")
            logger.info("=" * 60)
            sys.exit(0)
    ...
    async def run_vertex_batch_training(self, args):
        ...
        logger.info("=" * 60)
        logger.info("Vertex AI Batch Training Mode - FLUX.1")
        logger.info("=" * 60)
        ...
        try:
            ...
            logger.info("=" * 60)
            logger.info("FLUX.1 Training completed successfully!")
...
def main():
    """Main entry point for the FLUX.1 API."""
    import argparse

    parser = argparse.ArgumentParser(
        description="SimpleTuner FLUX.1 Training - API or Batch Mode"
    )
    ...
    logger.info("=" * 60)
    logger.info("SimpleTuner FLUX.1 Training")
    logger.info("=" * 60)
