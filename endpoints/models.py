...
async def get_wallet(request: Request):
    ...
    try:
        ...
        if hasattr(agent, 'wallet') and agent.wallet:
            ...
            all_balances = wallet.get_all_balances() if hasattr(wallet, 'get_all_balances') else {}
            return {
                "balance": float(fil_balance),
                "audit_reserve": float(wallet.get_audit_balance()) if hasattr(wallet, 'get_audit_balance') else 0,
                "total": float(fil_balance),
                "total_usd": float(usd_balance),
                "currency": "FIL",
                "balances": all_balances,
            }
        else:
            return {"balance": 0, "audit_reserve": 0, "total": 0, "total_usd": 0, "currency": "FIL"}
    except Exception as e:
...
async def get_keys(request: Request):
    ...
    try:
        ...
        from security.service_key_storage import ServiceKeyStorage
