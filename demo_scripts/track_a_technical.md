...
### Quick check — if you're picking up where you left off

Run this first. If both return OK, jump straight to Step 4 (load key):

```powershell
# Is Kestrel already running?
Invoke-RestMethod http://localhost:8888/health

# Is Ollama already running?
try { $ol = Invoke-RestMethod http://localhost:11434/api/version -TimeoutSec 3; Write-Host "Ollama UP: $($ol.version)" } catch { Write-Host "Ollama DOWN — start it (Step 1)" }
```
...
> **Note:** EPHEMERAL privacy mode routes LLM calls to local-only providers by design. If Ollama is not running, Act 4 returns a 500 error. Pull a model if needed: `ollama pull llama3.2:1b`
...
### 8. Position terminal for the demo

Font size 20+. Light theme if projecting onto a screen. Maximize the terminal window.
...
>
> The agent I've been running all week was born the same way — here's its identity chain right now."
...
| Problem | Recovery |
|---------|----------|
| `inception_service` command fails | Skip to Act 2 — say "That agent was born this way" — show DID via `Invoke-RestMethod -Uri "http://localhost:8888/api/agents/Kestrel/api/identity-chain" -Headers $headers \| ConvertTo-Json -Depth 3` |
