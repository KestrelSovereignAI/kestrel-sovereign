...
class TestProxyRequest:
    """Tests for proxy_request using httpx mock transport."""

    @pytest.fixture
    def rookery_config(self):
        """Create a rookery config with one local agent."""
    ...
    @pytest.mark.asyncio
    async def test_proxy_unknown_agent_returns_404(self, rookery_config):
        """Proxy to unknown agent returns 404."""
        scope = {
            "type": "http",
            "method": "GET",
            "path": "/api/agents/unknown/health",
            "query_string": b"",
            "headers": [],
        }
    ...
    @pytest.mark.asyncio
    async def test_proxy_offline_agent_returns_503(self, rookery_config):
        """Proxy to offline agent returns 503."""
        scope = {
            "type": "http",
            "method": "GET",
            "path": "/api/agents/claw/health",
            "query_string": b"",
            "headers": [],
        }
        from starlette.requests import Request as StarletteRequest
        ...
        # Agent port 9901 is not running — will get ConnectError
        async with httpx.AsyncClient() as client:
            response = await proxy_request(
                request=request,
                agent_id="claw",
        ...
        assert b"offline" in response.body

    @pytest.mark.asyncio
    async def test_proxy_successful_request(self, rookery_config):
        """Proxy forwards request and returns agent response."""

        async def mock_handler(request: httpx.Request) -> httpx.Response:
            assert str(request.url) == "http://localhost:9901/api/conversations"
        ...
        scope = {
            "type": "http",
            "method": "GET",
            "path": "/api/agents/claw/conversations",
            "query_string": b"",
            "headers": [],
        }
        from starlette.requests import Request as StarletteRequest
        ...
        assert b"conversations" in response.body

    @pytest.mark.asyncio
    async def test_proxy_preserves_query_string(self, rookery_config):
        """Proxy forwards query parameters to the agent."""
        captured_url = None
        ...
        scope = {
            "type": "http",
            "method": "GET",
            "path": "/api/agents/claw/api/search",
        ...
        assert "q=hello" in captured_url
        assert "limit=10" in captured_url

    @pytest.mark.asyncio
    async def test_proxy_forwards_post_body(self, rookery_config):
        """Proxy forwards request body for POST requests."""
        captured_body = None
        ...
        assert captured_body == body_content

    @pytest.mark.asyncio
    async def test_proxy_forwards_auth_headers(self, rookery_config):
        """Proxy forwards auth headers to agent."""
        ...
        assert captured_headers.get("authorization") == "Bearer token-abc"
