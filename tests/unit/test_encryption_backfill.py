...
class TestConversationHistoryBackfill:
    ...
    def test_retagging_rows_makes_the_next_boot_rebuild_the_session_projection(
        self, tmp_path, data_key
    ):
        """Re-tagging ``agent_id`` re-homes rows the #2959 projection groups by.

        ``conversation_sessions`` is keyed ``(agent_id, session_id)`` and may be
        absent or right, never stale. This tool moves a legacy untagged row onto
        the canonical agent, so a projection row written before the move counts
        rows that have left it — the one state that table may not be in. It
        cannot maintain the projection itself (raw ``sqlite3``, offline, no event
        loop), so it clears the completion marker and the rows and lets the next
        boot's backfill — which is also the repair pass — rebuild them.

        A dry run must leave both alone: it moves nothing.
        """
        db = tmp_path / "retag.db"
        conn = sqlite3.connect(str(db))
        conn.executescript(
            """
            CREATE TABLE graph_nodes (
        ...
        session_id = "3a6c1f0e-2b7d-4c8a-9e10-00000000002959"
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO graph_nodes (node_id, node_type, label, properties) "
            "VALUES (?, 'agent', 'Test', '{}')",
        ...
        cur.execute(
            "INSERT INTO conversation_history "
            "(agent_id, role, content, metadata, session_id) "
            "VALUES ('', 'user', 'legacy untagged turn', ?, ?)",
            (json.dumps({"session_id": session_id}), session_id),
        )
        # The projection as a prior boot left it: filed under the EMPTY agent,
        # which is the row the re-tag below invalidates.
        cur.execute(
            "INSERT INTO conversation_sessions (agent_id, session_id, "
            "message_count, user_message_count, first_user_message_id) "
            "VALUES ('', ?, 1, 1, 1)",
            (session_id,),
        )
        cur.execute(
            "INSERT INTO schema_backfills (name) VALUES "
            "('conversation_sessions_2959')"
        )
        conn.commit()
        conn.close()
        ...
        assert state() == (1, 1)
        backfill_conversation_history(db, agent_id=AGENT_DID, dry_run=True)
        assert state() == (1, 1), "a dry run moved nothing and must invalidate nothing"

        report = backfill_conversation_history(db, agent_id=AGENT_DID, dry_run=False)
        assert report.encrypted_now == 1
        assert state() == (0, 0), (
            "the marker still says the projection is built, so no boot will "
            "revisit the session this pass re-homed"
        )

    def test_retagging_survives_a_database_with_no_projection_tables(
        self, tmp_path, data_key
    ):
        """A database predating the projection has none to invalidate.

        The tables arrive with the schema, so an older file simply has nothing
        here to be wrong — and an encryption migration must not fail over a
        derived index it cannot find.
        """
        db = tmp_path / "no-projection.db"
        conn = sqlite3.connect(str(db))
        conn.executescript(
            """
            CREATE TABLE graph_nodes (
