...
INFERENCE_ONTOLOGY = OntologyRef(
...
@pytest.mark.asyncio
async def test_shacl_validation_loss_deactivates_only_affected_inference_proofs() -> None:
    """A SHACL repair retracts no alternate-supported conclusion before sleep."""
    ...
    def assertion(revision_id: str, subject: IRI, predicate: IRI, object_: IRI) -> Assertion:
        return Assertion(
            tenant_id=tenant,
    ...
    async def put(revision_id: str, subject: IRI, predicate: IRI, object_: IRI) -> Assertion:
    ...
    try:
        ...
        primary = await put("p-sub-q", property_p, RDFS_SUBPROPERTY, property_q)
        ...
        profile = InferenceProfile(INFERENCE_ONTOLOGY, "1.0.0")
        inference = BoundedInferenceService(storage._assertion_store(), profile)
        ...
        # Target exactly the primary proof's immutable revision node.  This
        # exercises the real SHACL revalidation service without invalidating
        # the independent P→R→Q proof.
        shapes = f"""
        @prefix ex: <https://example.test/> .
        @prefix sh: <http://www.w3.org/ns/shacl#> .
        ex:RejectPrimary a sh:NodeShape ;
          sh:targetNode <urn:kestrel:semantic-revision:{primary.revision_id}> ;
          sh:property [ sh:path ex:required ; sh:minCount 1 ] .
        """
        ...
        report = await validation.validate_current(
            assertion_ids=(primary.assertion_id,),
            shape_set=ShapeSetReference("test-shapes", "1.0.0"),
            validation_capability="validation-profile:test-core",
        )
        ...
        explanations = await inference.explain(conclusion.assertion_id)
        assert explanations
        assert all(primary.revision_id not in item.premise_revision_ids for item in explanations)
