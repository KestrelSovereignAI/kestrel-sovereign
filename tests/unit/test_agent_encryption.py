...
class TestKeyDerivation:
    ...
    def test_deterministic_key_derivation(self):
        ...
        key1 = get_agent_key(EMMA_DID, "service-keys")
        key2 = get_agent_key(EMMA_DID, "service-keys")
        assert key1 == key2
    ...
    def test_different_purposes_different_keys(self):
        ...
        key1 = get_agent_key(EMMA_DID, "service-keys")
        key2 = get_agent_key(EMMA_DID, "conversations")
        assert key1 != key2

    def test_all_purposes_produce_unique_keys(self):
        """All four purposes produce unique keys for the same agent."""
        keys = [get_agent_key(EMMA_DID, purpose) for purpose in VALID_PURPOSES]
    ...
    def test_key_length_is_32_bytes(self):
        ...
        for purpose in VALID_PURPOSES:
            key = get_agent_key(EMMA_DID, purpose)
            assert len(key) == 32
...
class TestEncryptDecrypt:
    ...
    def test_encrypt_decrypt_roundtrip(self):
        ...
        ciphertext = encrypt(EMMA_DID, "service-keys", plaintext)
        decrypted = decrypt(EMMA_DID, "service-keys", ciphertext)
    ...
    def test_encrypt_decrypt_string_roundtrip(self):
        ...
        ciphertext = encrypt_string(EMMA_DID, "service-keys", plaintext)
        decrypted = decrypt_string(EMMA_DID, "service-keys", ciphertext)
        assert decrypted == plaintext
    ...
    def test_encryption_produces_different_ciphertext_each_time(self):
        ...
        c1 = encrypt(EMMA_DID, "service-keys", plaintext)
        c2 = encrypt(EMMA_DID, "service-keys", plaintext)
        assert c1 != c2  # Different random nonces

        # But both decrypt to same plaintext
        assert decrypt(EMMA_DID, "service-keys", c1) == plaintext
        assert decrypt(EMMA_DID, "service-keys", c2) == plaintext
    ...
    def test_wrong_purpose_cannot_decrypt(self):
        ...
        with pytest.raises(DecryptionError):
            decrypt(EMMA_DID, "conversations", ciphertext)
...
class TestErrorHandling:
    ...
    def test_no_master_key_raises_error(self, monkeypatch):
        ...
        with pytest.raises(MasterKeyNotConfiguredError):
            get_agent_key(EMMA_DID, "service-keys")
    ...
    def test_decrypt_corrupted_data_raises_error(self):
        ...
        with pytest.raises(DecryptionError):
            decrypt(EMMA_DID, "service-keys", corrupted)
...
class TestAllPurposes:
    ...
    def test_purpose_encrypt_decrypt(self, purpose):
        ...
        ciphertext = encrypt(EMMA_DID, purpose, plaintext)
        decrypted = decrypt(EMMA_DID, purpose, ciphertext)
        assert decrypted == plaintext
