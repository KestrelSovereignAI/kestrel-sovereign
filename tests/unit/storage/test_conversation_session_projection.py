...
async def test_a_projection_row_is_dropped_when_its_session_stops_existing(seeded):
    ...
    **Three rows, and the third is #3120's.** B is a marker, a human turn, and
    ...
    take two: it resolved a canonical session by the rows that NAME it, so it
    left the inherited row live to reappear under a session of its own. "Every
    row of a session" is the list's answer or it is nothing.
    """
    db, store, projection = seeded
    assert await projection.get(UUID_B) is not None
