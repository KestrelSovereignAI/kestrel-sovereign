...
def test_contract_created_at(value, expected):
...
async def test_reindex_rewrites_bare_date_and_empty_stamps(tmp_path):
    ...
    try:
        ...
        # the minting helpers apply only to rows without one).
        ...
        old = {
            pat_1: (PATTERN_NODE_TYPE, "2026-07-01"),
            pat_2: (PATTERN_NODE_TYPE, ""),
            blk_1: (BLOCKER_NODE_TYPE, "2026-07-02"),
            dec_1: ("decision", "2026-07-03"),
        }
        for node_id, (node_type, stamp) in old.items():
            await graph.add_node(GraphNode(
                node_id=node_id, node_type=node_type, label="old",
                properties={"agent_id": AGENT, "created_at": stamp, "source": "strategic_memory"},
            ))

        await project_ledger(graph, AGENT, {PATTERNS_KEY: [pattern_row, empty_row], BLOCKERS_KEY: [blocker_row]})
