...
@pytest.mark.asyncio
async def test_get_agent_by_did_uses_atomic_hold_initialization(
    monkeypatch,
) -> None:
    ...
    agent = await main_module.get_agent_by_did("did:test:legacy")

    assert events == ["initialized"]
    assert agent._standalone_hold_context is context
...
@pytest.mark.asyncio
async def test_cli_shell_prints_typed_hold_refusal(
    monkeypatch,
    tmp_path,
    capsys,
) -> None:
    ...
    import kestrel_sovereign.llm.service as llm_module
    ...
    class _Storage:
        ...
        async def get_nodes_by_type(self, _node_type: str):
            return [SimpleNamespace(node_id="did:test:held")]
    ...
    agent = SimpleNamespace(
        ...
        process_input=AsyncMock(side_effect=refusal),
        shutdown=AsyncMock(),
    )
    close_context = AsyncMock()
    inputs = iter(["held", "!quit"])

    monkeypatch.setattr(storage_module, "AsyncStorage", _Storage)
    monkeypatch.setattr(agent_module, "KestrelAgent", lambda **_kwargs: agent)
    monkeypatch.setattr(
        agent_module, "await_agent_shutdown_completion", AsyncMock(return_value=False)
    )
    monkeypatch.setattr(llm_module, "LLMService", lambda: object())
    monkeypatch.setattr(
        hold_module,
        "initialize_with_bound_hold_context",
        initialize_with_hold,
...
async def test_streaming_input_refuses_before_first_yield_or_side_effect() -> None:
    ...
    agent_hold = _latch(HoldScope.AGENT, "hold:stream", target="did:test:held")
...
async def test_hold_snapshot_is_the_turn_admission_linearization_point() -> None:
    observed = _latch(HoldScope.AGENT, "hold:observed", target="did:test:held")
...
def test_bridge_surfaces_preserve_typed_hold_refusal() -> None:
    ...
    assert agent._cleanup_cancelled_request.call_count == 2
    cleaned_request_ids = {
        call.args[0] for call in agent._cleanup_cancelled_request.call_args_list
    }
    assert len(cleaned_request_ids) == 2
    assert all(
        isinstance(request_id, str) and request_id
        for request_id in cleaned_request_ids
    )
