...
def get_api_key():
    """Get or generate the API key, persisting it to .env if newly generated."""
    ...
    if not api_key:
        ...
        try:
            ...
            logger.info(f"âœ… KESTREL_API_KEY generated and saved to .env: {generated_key}")
        except OSError as e:
            logger.warning(f"âš ï¸  Could not persist KESTREL_API_KEY to .env: {e}")
            logger.warning(f"âš ï¸  Add this to your .env manually: KESTREL_API_KEY={generated_key}")
...
app.include_router(rasa_shim_router)  # Rasa-compatible shim for RemoteCares SMS (issue jaslogic1/RemoteCares#42)


@app.middleware("http")
async def auth_middleware(request: Request, call_next):
    """Global authentication middleware."""
    public_paths = ["/health", "/", "/api/auth/key", "/api/models", "/api/model/current", "/api/identity", "/api/commands", "/favicon.ico", "/webhooks/stripe/crypto", "/webhooks/rest/webhook"]
...
def get_stripe_webhook_handler():
    ...
    if _stripe_webhook_handler is None:
        try:
            from features.wallet.onramp import StripeOnRamp, StripeWebhookHandler
            onramp = StripeOnRamp()
