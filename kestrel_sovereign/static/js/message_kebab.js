 ...
 */
export function messageMenuItems(msg, node) {
    const id = msg && msg.id;
    const trash = {
 ...
 */
export function buildMessageKebab(msg, node) {
    const kebab = createKebabButton(() => messageMenuItems(msg, node), {
    ...
    if (node && typeof node.addEventListener === 'function') {
        node.addEventListener('contextmenu', (e) => {
            ...
            openMenuAt(messageMenuItems(msg, node), positionFromEvent(e));
        });
    }
    return kebab;
}
