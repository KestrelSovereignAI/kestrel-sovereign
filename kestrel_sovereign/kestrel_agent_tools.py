...
class AgentToolMixin:
    ...
    async def _handle_tool_commands(self, user_input: str) -> Optional[str]:
        ...
        # MCP Commands - Delegated to MCPAgent
        if command == "!mcp-load":
            if len(parts) < 2:
        ...
        # Model Management Commands - Delegated to ModelAgent
        ...
        if command == "!storage-status":
            try:
                ...
                return f"Storage Status:\nTotal: {info.get('total_gb', 0):.2f} GB\nUsed: {info.get('used_gb', 0):.2f} GB\nFree: {info.get('free_gb', 0):.2f} GB"
            except Exception as e:
                return f"❌ Error getting storage status: {str(e)}"

        if command == "!cleanup-models":
            try:
                result = await self.model_agent.cleanup_models()
                return f"Cleanup Result:\nFreed: {result.get('freed_space_gb', 0):.2f} GB\nRemoved: {', '.join(result.get('removed_models', []))}"
            except Exception as e:
        ...
        # Web search command
        if command == "!search" or command == "!web-search":
