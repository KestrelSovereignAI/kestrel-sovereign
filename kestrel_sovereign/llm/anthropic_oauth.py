...
class KeychainCredentialSource(CredentialSource):
    ...
    def _resolve_account(self) -> str:
        ...
        if best_account is not None:
            # Say so when the winner is already expired. It may still be
            # refreshable, so this is not a refusal — but binding is cached
            # for the life of this source, and an expired winner means every
            # other item was older or unparseable, which is worth one line
            # rather than a silent bind.
            if best_expires is not None and best_expires <= time.time():
                logger.warning(
                    "Claude OAuth: freshest keychain item %r is itself expired "
                    "(exp=%s); binding to it and relying on refresh",
                    best_account, best_expires,
                )
            self._account = best_account
            return best_account
        # Fallback: the default item's account attribute. Reached only when
        # enumeration produced nothing usable — including when
        # ``_list_service_accounts`` fails outright, since ``_run`` returns
        # None on a timeout or OS error. The default item is whichever the
        # keychain returns first, NOT necessarily the live login, so when the
        # keychain holds a stale login beside the current one this can bind a
        # process to a dead credential for its whole lifetime. That is
        # invisible downstream: Anthropic answers a bad plan credential with a
        # billing message naming no account, so one agent fails while another
        # in the same process succeeds. Observed 2026-09-06.
        #
        # Still taken — refusing to resolve would break the single-item case
        # that works — but never silently.
        ...
        if result is not None and result.returncode == 0:
            ...
            if m and m.group(1):
                logger.warning(
                    "Claude OAuth: keychain enumeration yielded no usable item; "
                    "falling back to the default item %r, which is not "
                    "necessarily the live login",
                    m.group(1),
                )
