...
_LOCAL_EMBEDDING_DESTINATION = "local"
...
class SemanticVectorProfile:
    ...
    embedding_destination: str = _LOCAL_EMBEDDING_DESTINATION
    rebuild_page_size: int = 100
    max_rebuild_rows: int = 2_000
    max_rebuild_seconds: float = 30.0
    ...
    def __post_init__(self) -> None:
        ...
        if self.embedding_destination not in {_LOCAL_EMBEDDING_DESTINATION, "remote"}:
            raise ValueError("semantic vector embedding_destination is invalid")
        if type(self.rebuild_page_size) is not int or not 1 <= self.rebuild_page_size <= 500:
            raise ValueError("semantic vector rebuild_page_size is outside the supported bound")
        if type(self.max_rebuild_rows) is not int or not 1 <= self.max_rebuild_rows <= 10_000:
            raise ValueError("semantic vector max_rebuild_rows is outside the supported bound")
        if self.rebuild_page_size > self.max_rebuild_rows:
            raise ValueError("semantic vector rebuild_page_size exceeds max_rebuild_rows")
        if (
            not isinstance(self.max_rebuild_seconds, (int, float))
            or isinstance(self.max_rebuild_seconds, bool)
            or not 0 < float(self.max_rebuild_seconds) <= 300
        ):
            raise ValueError("semantic vector max_rebuild_seconds is outside the supported bound")
...
def _normalise_vector(value: Sequence[float], *, dimension: int) -> tuple[float, ...]:
    ...
    if len(value) != dimension or len(value) > _MAX_VECTOR_DIMENSION:
...
class SemanticAssertionVectorProjection:
    ...
    async def _embed(self, assertion: Assertion) -> tuple[float, ...]:
        ...
        # Private/tenant/delegated assertions never cross it; enforce this
        # before rendering so sensitive bytes cannot reach the embedder even
    ...
    def _entry_values(
        ...
        return (
            ...
            self._profile.embedding_destination,
        ...
        async with self._store._mutation():  # noqa: SLF001 - canonical fence is the projection contract
            ...
            if vector is not None and current_eligible:
                await self._store._database.execute(  # noqa: SLF001
                    ...
                    "embedding_dimension, renderer_version, embedding_destination, visibility_ceiling, privacy_ceiling, visibility, privacy_classification, "
    ...
    async def _wipe_for_erasure(
        ...
        try:
            async with asyncio.timeout(float(self._profile.max_rebuild_seconds)):
                accepted_after = 0
                while True:
                    page = await self._eligible_current_assertion_page(
                        accepted_after=accepted_after,
                        limit=self._profile.rebuild_page_size,
                    )
                    if not page:
                        break
                    if len(projected) + len(page) > self._profile.max_rebuild_rows:
                        raise SemanticVectorProjectionError(
                            "semantic vector erasure rebuild row budget exceeded"
                        )
                    for assertion, accepted_order in page:
                        self._assert_privacy_scope(assertion)
                        projected.append((assertion, await self._embed(assertion)))
                        accepted_after = accepted_order
                    if len(page) < self._profile.rebuild_page_size:
                        break
        except TimeoutError as error:
            raise SemanticVectorProjectionError(
                "semantic vector erasure rebuild time budget exceeded"
            ) from error
    ...
    async def _eligible_current_assertion_page(
        self, *, accepted_after: int, limit: int,
    ) -> tuple[tuple[Assertion, int], ...]:
        """Read one bounded keyset page for an opaque-erasure rebuild."""
        rows = await self._store._database.fetchall(  # noqa: SLF001 - canonical projection read
            "SELECT r.assertion_mapping, r.accepted_order FROM semantic_assertions a "
            ...
            "AND r.accepted_order > ? ORDER BY r.accepted_order ASC, r.revision_id ASC LIMIT ?",
            (self._store.tenant_id, AssertionStatus.ACTIVE.value, accepted_after, limit),
        )
        return tuple(
            (Assertion.from_mapping(json.loads(row[0])), int(row[1])) for row in rows
        )
    ...
    async def _advance(
        ...
        cursor_predicate = (
            ...
            else "checkpoint_event_id = ?"
        )
        params: tuple[object, ...] = (
            generation, event_id, self._store.tenant_id, self._profile.profile_id,
            self._profile.capability_digest, expected.generation,
        )
        if expected.event_id is not None:
            params += (expected.event_id,)
        changed = await self._store._database.execute(  # noqa: SLF001
            ...
            "WHERE tenant_id = ? AND profile_id = ? AND capability_digest = ? "
            f"AND checkpoint_generation = ? AND {cursor_predicate}",
            params,
        )
        if changed != 1:
            raise SemanticVectorProjectionError("semantic_vector_projection_concurrent_progress")
    ...
    async def recall(
        ...
        rows = await self._store._database.fetchall(  # noqa: SLF001
            ...
            "v.embedding_provider, v.embedding_model, v.embedding_dimension, v.renderer_version, v.embedding_destination, "
        ...
        for (
            ...
            renderer_version, embedding_destination, visibility_ceiling, privacy_ceiling, visibility, privacy_classification,
            ...
            if (
                ...
                or embedding_destination != self._profile.embedding_destination
