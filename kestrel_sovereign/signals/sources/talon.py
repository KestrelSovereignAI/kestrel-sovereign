...
def _talon_job_complete_result_summary(body: Any) -> str:
    ...
    string — the turn the job completion woke. Surfacing it here is what lets
    ...
    non-empty ``result_summary``, and with the callback absent this rail
    emitted metadata only, so the wake stayed invisible until a manual
    refresh even once it landed in the right session.
...
def build_talon_job_complete_registration() -> SourceRegistration:
    return SourceRegistration(
        ...
        # the dispatcher never reaches this callback's emit.
