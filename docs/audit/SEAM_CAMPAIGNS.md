...
| Export/import with encryption, key rotation, and storage receipts | encrypted conversation preview; identity export/import; sovereignty export/import; key resolution | encrypted message rows, fake Fernet decryptor, identity/sovereignty fixtures, copied receipt node writes | encrypted data is not silently downgraded to plaintext; failed decrypt remains visibly encrypted; sovereignty receipts preserve storage provenance, encryption-key identity, and audit anchors before persistence | `tests/unit/test_commands_conversations_endpoint_contracts.py`, `tests/unit/test_sovereignty_export_receipt_contracts.py`, `tests/integration/test_identity_export_import.py`, `tests/integration/test_sovereignty_e2e.py` | #595 | Partial |
...
3. Export/import still needs missing-key and rotated-key negative cases; receipt provenance is now covered, but decrypt failures must prove they fail closed without metadata mutation.
