...
| Mandate and fallback routing across UI, API, command, and runtime | discovered default model ranking; explicit provider/model preference; UI/API/command current-model convergence; available-mandate vs configured fallback; unavailable-mandate degrades to fallback; unavailable-mandate with no fallback raises (no blind vendor swap); UI endpoint rewrite | model catalog fixtures, API client storage/fetch stubs, fake providers, mandate+fallback `LLMService` fixture | no hardcoded current model names in generated docs; generated docs resolve `auto` through the discovery cache; UI/API/command current-model paths agree via `get_model_preference()`; an available explicit preference is used verbatim and is never overridden by a configured fallback; fallback engages only when the mandated route is unavailable, else raises rather than silently swapping vendors; browser rewrites do not route host-auth endpoints into agent space | `tests/unit/test_model_selection_contracts.py`, `tests/unit/test_generate_feature_docs.py`, `tests/unit/test_feature_doc_canonicality.py`, `tests/unit/test_current_model_selection_seams.py`, `tests/unit/test_agent_backend_routing_contracts.py`, `tests/frontend/api_client.test.mjs` | #592 | Proven |
...
| Permission bypass through MCP, A2A, tools, and commands | command-triggered privileged action vs tool/A2A-triggered action; compute approval boundaries; stale approval; replayed approval; forged/blank permission scope; extracted MCP boundary | fake task store, shared compute approval-boundary fixture, permission-store + approval-queue fixture, core-only agent fixtures, optional feature entry-point fixtures | privileged operations cannot bypass approval through alternate protocol paths; stale/replayed approvals fail before side effects; a forged/blank scope cannot inherit a privileged allow and falls to the default ASK rail (fails closed) with no silent auto-allow; task status is auditable; command handlers preserve async approval boundaries; core-only command discovery does not advertise extracted MCP commands | `tests/unit/test_permission_seam_contracts.py`, `tests/unit/test_extracted_feature_boundary_contracts.py`, `tests/unit/test_security_feature.py`, `tests/unit/test_agent_runtime_endpoint_contracts.py`, `tests/integration/test_compute_security_integration.py`, `tests/integration/test_core_only_boot.py` | #596 | Partial |
| Bootstrap/auth/session interactions in browser and SSE flows | localhost bootstrap; OAuth-required redirect; API key query auth limited to SSE; JWT expiry; stream retry after bootstrap refresh; deployed multi-agent SSE EventSource routing | real FastAPI app with lifespan disabled, TestClient, browser API client fetch/storage stubs | bootstrap key stays localhost-only and disabled under OAuth-required mode; query-string API keys authenticate only SSE paths; JWT expiry clears platform tokens and redirects; API-key 401 refreshes once and retries both JSON and stream requests | `tests/unit/test_auth_decision_table.py`, `tests/unit/test_sse_connection_limits.py`, `tests/frontend/api_client.test.mjs` | #255 | Proven |
...
1. Permission bypass through MCP, A2A, tools, and commands: the in-core shared approval/audit boundary is now proven — stale approval, replayed approval, forged/blank scope, A2A, command, and core-only MCP extraction paths. The campaign stays **Partial** because the MCP seam still lacks proof for a real installed `kestrel-feature-mcp` package invoking an external MCP tool path (only the extracted/core-only boundary is covered today); that remains the open seam before this row can move to Proven.
