...
| Permission bypass through MCP, A2A, tools, and commands | command-triggered privileged action vs tool/A2A-triggered action; compute approval boundaries; stale approval; replayed approval; malformed permission scope; extracted MCP boundary | fake task store, shared compute approval-boundary fixture, security hook/audit fixture, core-only agent fixtures, optional feature entry-point fixtures | privileged operations cannot bypass approval through alternate protocol paths; stale/replayed approvals fail before side effects; malformed scopes fail closed and audit denial; task status is auditable; command handlers preserve async approval boundaries; core-only command discovery does not advertise extracted MCP commands | `tests/unit/test_permission_seam_contracts.py`, `tests/unit/test_extracted_feature_boundary_contracts.py`, `tests/unit/test_security_feature.py`, `tests/unit/test_agent_runtime_endpoint_contracts.py`, `tests/integration/test_compute_security_integration.py`, `tests/integration/test_core_only_boot.py` | #596 | Proven |
...
1. Permission bypass through MCP, A2A, tools, and commands is proven for the in-core shared approval/audit boundary, including stale approval, replayed approval, malformed scope, A2A, command, and core-only MCP extraction paths.
