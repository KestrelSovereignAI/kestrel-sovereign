...
| Emma-named scripts (audited individually) | `scripts/provision_emma_openrouter.py`, `scripts/rotate_emma_key.py`, `scripts/emma_scheduler.py` | First is a redundant wrapper around [`scripts/manage_openrouter_keys.py`][mgmt]. Second is functionally generic, just badly named (default `--db` was Emma's). Third is one-off Emma scaffolding (born in initial commit, never modified, never invoked). Phase 0 handles all three differently. |
...
- `scripts/provision_emma_openrouter.py` — **delete**. It's a redundant
  wrapper around `manage_openrouter_keys.py provision --agent emma`,
  which does the exact same thing parameterized.
...
- `scripts/rotate_emma_key.py` — **rename to `rotate_agent_key.py`**.
  Functionally generic (argparse with `--db`, `--old-pass`, `--new-pass`,
  `--dry-run`), but the original default `--db` was Emma's database
  path. The rename also drops the default — `--db` is now required —
  and updates references in
...
**Codex review focus:** any caller I missed; whether `emma_scheduler.py`
genuinely has no live consumer (since "never invoked" is a hard claim).
