...
| [kestrel-feature-reflection](https://github.com/KestrelSovereignAI/kestrel-feature-reflection) ([AGENTS.md](https://github.com/KestrelSovereignAI/kestrel-feature-reflection/blob/main/AGENTS.md)) | Canonical owner of `ReflectionFeature`: agent self-reflection and self-improvement. |
| [kestrel-feature-intelligence](https://github.com/KestrelSovereignAI/kestrel-feature-intelligence) (archived, [AGENTS.md](https://github.com/KestrelSovereignAI/kestrel-feature-intelligence/blob/main/AGENTS.md)) | Legacy combined bundle; its stale Reflection/Council entry points conflict with the canonical standalone packages. Do not co-install it. |
| [kestrel-feature-parametric-self](https://github.com/KestrelSovereignAI/kestrel-feature-parametric-self) ([AGENTS.md](https://github.com/KestrelSovereignAI/kestrel-feature-parametric-self/blob/main/AGENTS.md)) | Owned parametric self — a per-agent local model nightly-finetuned on the agent's own experience. It remains an optional governed-corpus consumer; core records its cross-repository release evidence without importing its internals. |
...
processes. In governance mode it may publish a self-asserted `did:pkh`
instrument identity; that identity is not a Kestrel agent lineage node. A
Talon run has no `spawned_by` lineage graph edge and no verified parent-signed
spawn mandate or durable lineage receipt. The independently installed
`kestrel-feature-talon` package gives an agent a governed control surface for
that separate runner. Its terminal `talon.job_complete` enters the target
agent's causation chain and receives a `CausationFrame`; the frame attributes
the agent wake, not the Talon process. This does not turn Talon runs into agents
or bridge Talon process relationships onto either agent relationship axis.

| Repository | What it is |
|---|---|
| [kestrel-talon](https://github.com/KestrelSovereignAI/kestrel-talon) (private, [AGENTS.md](https://github.com/KestrelSovereignAI/kestrel-talon/blob/main/AGENTS.md)) | Standalone GitHub/ADO issue-processing engine used by the independently installed `kestrel-feature-talon` coordinator; its repository-run orchestration does not create an agent relationship. |
